import json
from app.llm_clients.adapter_registry import adapter_map
...
def get_embedding_function():
    for model_key in config["priority"]:
        ...
        try:
            model_name = model_conf["model_name"]
            return adapter_map[model_key]["get_embedding_function"](model=model_name)
        except Exception as e:
...
def get_embedding_model_and_config():
    for model_key in config["priority"]:
        ...
        try:
            ...
            print(f"[WARN] Falló función de configuración para {model_key}: {e}")
            if not config.get("fallback_enabled"):
                raise
